 ...
 * @brief 設定リロード（サービス）
 */
void FvAsparaAnalyzerNode::reloadConfigService(
    const std::shared_ptr<std_srvs::srv::Trigger::Request> request,
    std::shared_ptr<std_srvs::srv::Trigger::Response> response)
{
    (void)request;
    try {
        // 本ノードは描画・処理時に get_parameter で参照する設計のため、
        // サービスでは特別な再初期化は不要。必要に応じてキャッシュを更新する。

        // 将来のキャッシュ項目が増えた場合に備えて、代表的なUI系の値を一度読み出して
        // ログに出しておく（確認用）。
        double fs = this->has_parameter("info_font_scale") ? this->get_parameter("info_font_scale").as_double() : 0.5;
        int lh = this->has_parameter("info_line_height") ? this->get_parameter("info_line_height").as_int() : 18;
        double a = this->has_parameter("info_window_alpha") ? this->get_parameter("info_window_alpha").as_double() : 0.5;
        ...
        RCLCPP_INFO(this->get_logger(), "[reload_config] UI fs=%.2f lh=%d alpha=%.2f panels L=%d R=%d",
                    fs, lh, a, (int)left_on, (int)right_on);

        // 解析スレッド等にフラグ通知が必要ならここで行う（必要になれば追加）

        response->success = true;
        response->message = "fv_aspara_analyzer: パラメータを再読込しました (動的反映)";
    } catch (const std::exception& e) {
        response->success = false;
        response->message = std::string("reload_config 失敗: ") + e.what();
    }
}

std::string FvAsparaAnalyzerNode::buildSelectedAsparaJson(const AsparaInfo &a, const std::string &camera_prefix)
{
    auto sane = [](double v){ return std::isfinite(v) ? v : 0.0; };
    std::ostringstream os;
    os.setf(std::ios::fixed);
    os << "{";
    if (!camera_prefix.empty()) os << "\"camera\":\"" << camera_prefix << "\",";
    os << "\"id\":" << a.id << ","
       << "\"confidence\":" << a.confidence << ","
       << "\"grade\":" << static_cast<int>(a.grade) << ","
       << "\"is_harvestable\":" << (a.is_harvestable ? "true" : "false") << ","
       << "\"length_m\":" << a.length << ","
       << "\"diameter_m\":" << a.diameter << ","
       << "\"straightness\":" << a.straightness << ","
       << "\"curvature_m\":" << a.curvature << ","
       << "\"distance_from_camera_m\":" << a.distance_from_camera << ","
       << "\"bbox\":{\"x\":" << a.bounding_box_2d.x << ",\"y\":" << a.bounding_box_2d.y
       << ",\"w\":" << a.bounding_box_2d.width << ",\"h\":" << a.bounding_box_2d.height << "},"
       << "\"root_world\":{\"x\":" << sane(a.root_position_3d.x) << ",\"y\":" << sane(a.root_position_3d.y)
       << ",\"z\":" << sane(a.root_position_3d.z) << "},"
       << "\"approach\":{\"pixel\":{\"x\":" << a.approach.pixel.x << ",\"y\":" << a.approach.pixel.y << "},"
       << "\"world\":{\"x\":" << sane(a.approach.world.x) << ",\"y\":" << sane(a.approach.world.y)
       << ",\"z\":" << sane(a.approach.world.z) << "},\"valid\":" << (a.approach.world_valid?"true":"false") << "},"
       << "\"skeleton_count\":" << a.skeleton_points.size() << "}";
    return os.str();
}
...
FvAsparaAnalyzerNode::FvAsparaAnalyzerNode() : Node("fv_aspara_analyzer")
{
    ...
    // 可視化委譲（既定でON）
    this->declare_parameter<bool>("use_visualizer_delegate", true);
    this->declare_parameter<bool>("mask_overlay_enabled", false);
    // AsparaVoxelWindow 設定
    this->declare_parameter<std::string>("voxel_window.position", "left"); // left | right
    this->declare_parameter<bool>("voxel_window.overlay_enabled", true);
    this->declare_parameter<double>("voxel_window.overlay_alpha", 0.5);
    this->declare_parameter<bool>("voxel_window.straighten", false);
    this->declare_parameter<double>("voxel_window.zoom", 1.0); // 1.0=等倍
    this->declare_parameter<bool>("voxel_window.depth_background_enabled", false);
    // 選択情報出力
    this->declare_parameter<bool>("selected_asparagus_info_enabled", false);
    this->declare_parameter<std::string>("selected_asparagus_topic", "/selected_asparagus_info");
    this->declare_parameter<double>("selected_asparagus_publish_rate", 10.0);
    // 領域認識（色/セグ）ON/OFF
    ...
    // 選択情報パブリッシャとタイマー
    bool sel_enabled = this->get_parameter("selected_asparagus_info_enabled").as_bool();
    if (sel_enabled) {
        std::string sel_topic = this->get_parameter("selected_asparagus_topic").as_string();
        selected_info_pub_ = this->create_publisher<std_msgs::msg::String>(sel_topic, 10);
        double hz = std::max(1.0, this->get_parameter("selected_asparagus_publish_rate").as_double());
        selected_info_timer_ = this->create_wall_timer(
            std::chrono::milliseconds(static_cast<int>(1000.0 / hz)),
            [this]() {
                int sel_id = aspara_selection_.getSelectedAsparaId();
                if (sel_id < 0 || !selected_info_pub_) return;
                std::lock_guard<std::mutex> lk(aspara_list_mutex_);
                auto it = std::find_if(aspara_list_.begin(), aspara_list_.end(), [sel_id](const AsparaInfo& a){ return a.id == sel_id;});
                if (it == aspara_list_.end()) return;
                // カメラ名（D405/D415など）
                std::string cam = this->has_parameter("camera_name") ? this->get_parameter("camera_name").as_string() : std::string("");
                std_msgs::msg::String msg;
                msg.data = buildSelectedAsparaJson(*it, cam);
                selected_info_pub_->publish(msg);
            }
        );
    }

    // 可視化クラス初期化（描画・画像パブリッシュ委譲先）
    try {
        visualizer_ = std::make_unique<AsparaVisualizer>(this, &overlay_engine_);
        RCLCPP_INFO(this->get_logger(), "AsparaVisualizer initialized");
        // Overlay補間ゲインをYAMLから読み込んで反映
        try {
            double sg = this->declare_parameter<double>("overlay.smooth_gain", 12.0);
            double fg_in = this->declare_parameter<double>("overlay.fade_in_gain", 6.0);
            double fg_out = this->declare_parameter<double>("overlay.fade_out_gain", 3.0);
            overlay_engine_.setGains(sg, fg_in, fg_out);
        } catch (...) {}
    } catch (const std::exception& e) {
        RCLCPP_WARN(this->get_logger(), "AsparaVisualizer init failed: %s", e.what());
    }
    ...
    reload_config_service_ = this->create_service<std_srvs::srv::Trigger>(
        "~/reload_config",
        std::bind(&FvAsparaAnalyzerNode::reloadConfigService, this, std::placeholders::_1, std::placeholders::_2));
    ...
    // ===== 画面更新タイマー（既定15FPS） =====
    double ui_fps = 15.0;
    try { ui_fps = this->declare_parameter<double>("ui.fps", 15.0); } catch (...) {}
    int interval_ms = static_cast<int>(std::max(1.0, std::round(1000.0 / std::max(1.0, ui_fps))));
    auto timer_callback = [this, ui_fps]() {
        // アニメーション補間（ui.fpsに同期）
        try { overlay_engine_.tick(1.0 / std::max(1.0, ui_fps)); } catch (...) {}
        // 常時出力（画像未到着でも待機画面）
        publishCurrentImage();
    };
    animation_timer_ = flr::make_timer(this, std::chrono::milliseconds(interval_ms), timer_callback);
    RCLCPP_WARN(this->get_logger(), "Animation timer created (%.1f FPS, %dms)", ui_fps, interval_ms);
...
{
    // 可視化高速経路: Visualizerへ委譲（フラグ有効時）。既存の複雑な描画処理をスキップして軽量化。
    try {
        bool use_visualizer = this->has_parameter("use_visualizer_delegate") ? this->get_parameter("use_visualizer_delegate").as_bool() : true;
        if (use_visualizer && latest_color_image_) {
            ...
            if (this->get_parameter("mask_overlay_enabled").as_bool()) {
                applySegOverlay(color_image);
            }
            std::vector<AsparaInfo> snapshot_list; int snapshot_selected_id = -1;
            {
                std::lock_guard<std::mutex> lk(aspara_list_mutex_);
            ...
            cv::Mat rendered;
            if (visualizer_) {
                std::map<std::string, float> fps_info;
                rendered = visualizer_->renderCurrentImage(color_image, snapshot_list, snapshot_selected_id, fps_info, enable_pointcloud_processing_);
            }
            if (rendered.empty()) rendered = color_image;
            if (visualizer_) {
                visualizer_->publishImage(rendered, latest_color_image_->header, annotated_image_pub_);
            } else if (annotated_image_pub_) {
                auto msg = cv_bridge::CvImage(latest_color_image_->header, sensor_msgs::image_encodings::BGR8, rendered).toImageMsg();
                annotated_image_pub_->publish(*msg);
            }
            return;
        }
        // フォールバック: 画像が未到着でも“必ず”何かを出力（安定出力）
        if (use_visualizer && !latest_color_image_ && annotated_image_pub_) {
            int w = 640, h = 480;
            try { if (latest_camera_info_) { w = static_cast<int>(latest_camera_info_->width); h = static_cast<int>(latest_camera_info_->height); } } catch (...) {}
            w = std::clamp(w, 160, 1920); h = std::clamp(h, 120, 1080);
            ...
            std_msgs::msg::Header hdr; hdr.stamp = this->now(); hdr.frame_id = latest_camera_info_ ? latest_camera_info_->header.frame_id : std::string("camera");
            if (visualizer_) visualizer_->publishImage(standby, hdr, annotated_image_pub_);
            else {
                auto msg = cv_bridge::CvImage(hdr, sensor_msgs::image_encodings::BGR8, standby).toImageMsg();
                annotated_image_pub_->publish(*msg);
            }
            return;
        }
    ...
    // 画像のバタつき抑制: 古い検出を一定時間で非表示にする（デフォルト0.5s）
    double stale_sec = 0.5;
    try {
        if (this->has_parameter("overlay.stale_max_age_sec")) {
            stale_sec = std::max(0.0, this->get_parameter("overlay.stale_max_age_sec").as_double());
        }
    } catch (...) {}
    auto now_sys2 = std::chrono::steady_clock::now();
    double since_update_sec = std::chrono::duration<double>(now_sys2 - last_detection_update).count();
    if (since_update_sec > stale_sec) {
        snapshot_list.clear();
        snapshot_selected_id = -1;
    }
    ...
    if (!snapshot_list.empty()) {
        ...
        for (auto& aspara_info : snapshot_list) {
            ...
            // バウンディングボックス描画（グレード色）: 表示も計算も bounding_box_2d に統一
            cv::rectangle(output_image, aspara_info.bounding_box_2d, grade_color, thickness);

            // アプローチポイント（紫丸 + xyz）: approach.enable & approach.overlay.enable かつ pixel_valid の場合に描画
            try {
                bool enable_approach = true;
                bool overlay_enable = true;
                if (this->has_parameter("approach.enable")) enable_approach = this->get_parameter("approach.enable").as_bool();
                if (this->has_parameter("approach.overlay.enable")) overlay_enable = this->get_parameter("approach.overlay.enable").as_bool();
                if (enable_approach && overlay_enable && aspara_info.approach.pixel_valid) {
                    cv::Scalar color_bgr(255, 0, 255);
                    int radius = 6;
                    int thickness2 = 2; // 既定は輪
                    bool debug_xyz = true;
                    try {
                        // YAMLの値を確実に取得（描画属性のみ使用）
                        radius = this->declare_parameter<int>("approach.overlay.radius", 6);
                        thickness2 = this->declare_parameter<int>("approach.overlay.thickness", 2);
                        debug_xyz = this->declare_parameter<bool>("approach.overlay.debug_xyz", true);
                    } catch (...) {}
                    // 2Dは解析スレッドが保存した値をそのまま使用（再計算しない）
                    cv::Point2f px_draw = aspara_info.approach.pixel;
                    OverlayParams op{overlay_enable, color_bgr, radius, thickness2, debug_xyz, color_bgr, 0.5, 1};
                    drawApproachPoint(output_image, px_draw,
                                      &aspara_info.approach.world,
                                      aspara_info.approach.world_valid,
                                      op);
                }
            } catch (...) {}
        ...
        bool left_on  = this->has_parameter("left_panel_enabled") ? this->get_parameter("left_panel_enabled").as_bool() : false;
        bool right_on = this->has_parameter("right_panel_enabled") ? this->get_parameter("right_panel_enabled").as_bool() : true;
        // 新パラメータで上書き（位置を統一的に制御）
        std::string vw_pos = this->get_parameter("voxel_window.position").as_string();
        if (vw_pos == "left") { left_on = true; right_on = false; }
        else if (vw_pos == "right") { left_on = false; right_on = true; }
        if (snapshot_selected_id != -1 && (left_on || right_on)) {
            ...
            if (depth_copy && caminfo_copy) {
                ...
                if (it_sel != snapshot_list.end()) {
                    const cv::Rect roi = it_sel->bounding_box_2d & cv::Rect(0,0,output_image.cols, output_image.rows);
                    if (roi.area() > 0) {
                        try {
                            // 深度画像をOpenCVに
                            cv::Mat depth_mat;
                            ...
                            if (!depth_mat.empty()) {
                                ...
                                if (points.size() >= 20) {
                                    ...
                                    if (left_ok) {
                                        bool ov_on = this->get_parameter("voxel_window.overlay_enabled").as_bool();
                                        double ov_alpha = std::clamp(this->get_parameter("voxel_window.overlay_alpha").as_double(), 0.0, 1.0);
                                        if (ov_on) {
                                            ...
                                            cv::addWeighted(overlay_panel, ov_alpha, roi_img, 1.0 - ov_alpha, 0.0, roi_img);
                                        }
                                    }

                                    // Depth ROI (Jet) preview in left panel background (configurable)
                                    if (left_ok && this->get_parameter("voxel_window.depth_background_enabled").as_bool()) try {
                                    ...
                                    auto drawCloudToPanel = [&](const sensor_msgs::msg::PointCloud2& pc2,
                                        ...
                                        double vw_zoom = std::clamp(this->get_parameter("voxel_window.zoom").as_double(), 0.1, 10.0);
                                        auto putDot = [&](int px, int py, const cv::Vec3b& c){
                                            // ズーム（パネル中心基準）
                                            if (std::abs(vw_zoom - 1.0) > 1e-6) {
                                                double cxp = pnl_rect.x + pnl_rect.width * 0.5;
                                                double cyp = pnl_rect.y + pnl_rect.height * 0.5;
                                                px = static_cast<int>(std::round(cxp + (px - cxp) * vw_zoom));
                                                py = static_cast<int>(std::round(cyp + (py - cyp) * vw_zoom));
                                            }
                                    ...
                                    // PCAで整列して描画（真っ直ぐ表示）
                                    auto drawCloudToPanelStraight = [&](const sensor_msgs::msg::PointCloud2& pc2,
                                                                       const cv::Rect& pnl_rect,
                                                                       int& out_count) {
                                        out_count = 0;
                                        ...
                                        for (int i = 0; i < data.rows; ++i) {
                                         ...
                                         // 符号はカメラYとの相関で安定化（PC1↑で画面Y↓になるよう反転）+ ヒステリシス
                                         ...
                                         int curr_sgn = (corr > 0.0) ? -1 : 1; // corr>0 → 反転して上向き
                                         if (curr_sgn != last_sgn && std::abs(corr) < 0.6) {
                                             curr_sgn = last_sgn; // ヒステリシスでフリップ抑制
                                         } else {
                                             last_sgn = curr_sgn;
                                         }
                                         float sgn = static_cast<float>(curr_sgn);
                                        // 範囲
                                        double minx=1e9,maxx=-1e9,miny=1e9,maxy=-1e9;
                                        for (int i = 0; i < proj.rows; ++i) {
                                            double X = proj.at<float>(i,1); // 横=PC2
                                            double Y = sgn * proj.at<float>(i,0); // 縦=PC1（上向き）
                                    ...
                                    if (left_ok) {
                                        // 左枠: フィルタ済み点群（要求に合わせ左へ配置）。整列表示のオプション対応
                                        int drawn = 0;
                                        bool straighten = this->get_parameter("voxel_window.straighten").as_bool();
                                        if (it_sel != snapshot_list.end() && !it_sel->filtered_pointcloud.data.empty()) {
                                            if (straighten) drawCloudToPanelStraight(it_sel->filtered_pointcloud, panel, drawn);
                                            else            drawCloudToPanel(it_sel->filtered_pointcloud, roi, panel, drawn);
                                        }
                                        // 左枠ラベル（フィルタ済み点数）
                                        int filt_total = 0;
                                        if (it_sel != snapshot_list.end() && !it_sel->filtered_pointcloud.data.empty()) {
                                    ...
                                    if (right_ok) {
                                        ...
                                        if (ov_on) {
                                            ...
                                            cv::addWeighted(ov_r, ov_alpha, roi_r, 1.0 - ov_alpha, 0.0, roi_r);
                                        }
                                        // 右枠: FILTERED（filtered_pointcloud）。整列表示のオプション対応
                                        int filt_drawn = 0;
                                        bool straighten = this->get_parameter("voxel_window.straighten").as_bool();
                                        if (it_sel != snapshot_list.end() && !it_sel->filtered_pointcloud.data.empty()) {
                                            if (straighten) drawCloudToPanelStraight(it_sel->filtered_pointcloud, panel_r, filt_drawn);
                                            else            drawCloudToPanel(it_sel->filtered_pointcloud, roi, panel_r, filt_drawn);
                                        }
                                        ...
                                        try {
                                            ...
                                            if (show_straightness) lines.push_back(cv::format("曲がり度: %.1f", it_sel->straightness * 100.0));
                                    ...
                                    // 情報ウィンドウを右パネルに依存せずに描画（右パネルが出ていない場合も単独で表示）
                                    try {
                                        bool info_enable = this->has_parameter("info_window_enabled") ? this->get_parameter("info_window_enabled").as_bool() : true;
                                        if (info_enable && it_sel != snapshot_list.end()) {
                                            auto paramb = [this](const char* key, bool defv){return this->has_parameter(key)? this->get_parameter(key).as_bool() : defv;};
                                            bool show_id = paramb("info_show_id", true);
                                            bool show_conf = paramb("info_show_confidence", true);
                                            bool show_length = paramb("info_show_length", true);
                                            ...
                                            if (show_id) lines2.push_back(cv::format("ID: %d", it_sel->id));
                                            if (show_conf) lines2.push_back(cv::format("信頼度: %.0f%%", it_sel->confidence * 100.0));
                                            double disp_len_min = this->has_parameter("display_length_min_m") ? this->get_parameter("display_length_min_m").as_double() : 0.0;
                                            double disp_len_max = this->has_parameter("display_length_max_m") ? this->get_parameter("display_length_max_m").as_double() : std::numeric_limits<double>::infinity();
                                            double disp_dia_min = this->has_parameter("display_diameter_min_m") ? this->get_parameter("display_diameter_min_m").as_double() : 0.0;
                                            double disp_dia_max = this->has_parameter("display_diameter_max_m") ? this->get_parameter("display_diameter_max_m").as_double() : std::numeric_limits<double>::infinity();
                                            bool len_ok2 = it_sel->length_valid && it_sel->length >= disp_len_min && it_sel->length <= disp_len_max;
                                            bool dia_ok2 = (it_sel->diameter >= disp_dia_min && it_sel->diameter <= disp_dia_max);
                                            if (show_length) lines2.push_back(len_ok2 ? cv::format("長さ: %.1fcm", it_sel->length * 100.0) : std::string("長さ: --"));
                                            if (show_diameter) lines2.push_back(dia_ok2 && it_sel->diameter > 0 ? cv::format("太さ: %.0fmm", it_sel->diameter * 1000.0) : std::string("太さ: --"));
                                            if (show_straightness) lines2.push_back(cv::format("曲がり度: %.1f", it_sel->straightness * 100.0));
                                            if (show_debug) {
                                                double chord_cm2 = 0.0;
                                                if (!it_sel->skeleton_points.empty()) {
                                                    const auto &wf = it_sel->skeleton_points.front().world_point;
                                                    const auto &wl = it_sel->skeleton_points.back().world_point;
                                                    double dx = static_cast<double>(wf.x) - static_cast<double>(wl.x);
                                                    double dy = static_cast<double>(wf.y) - static_cast<double>(wl.y);
                                                    double dz = static_cast<double>(wf.z) - static_cast<double>(wl.z);
                                                    chord_cm2 = std::sqrt(dx*dx + dy*dy + dz*dz) * 100.0;
                                                }
                                                lines2.push_back(cv::format("参照直線: %.1fcm", chord_cm2));
                                            }
                                            if (show_grade) {
                                                const char* gl = (it_sel->grade==AsparaguGrade::A_GRADE?"A": it_sel->grade==AsparaguGrade::B_GRADE?"B": it_sel->grade==AsparaguGrade::C_GRADE?"C":"NG");
                                                lines2.push_back(std::string("グレード: ") + gl);
                                            }
                                            if (show_harvest) lines2.push_back(std::string("収穫可否: ") + (it_sel->is_harvestable?"YES":"NO"));
                                            if (show_distance) {
                                                double dm2 = 0.0; const auto &rp2 = it_sel->root_position_3d;
                                                if (std::isfinite(rp2.x) && std::isfinite(rp2.y) && std::isfinite(rp2.z)) dm2 = std::sqrt(rp2.x*rp2.x + rp2.y*rp2.y + rp2.z*rp2.z);
                                                if (dm2 > 0.0) lines2.push_back(cv::format("距離: %.2fm", dm2));
                                            }

                                            const int pad2 = 6; const int lh2 = 18; int panel_text_w2 = 0; int base2 = 0;
                                            for (const auto &ln : lines2) { cv::Size ts = cv::getTextSize(ln, cv::FONT_HERSHEY_SIMPLEX, 0.5, 1, &base2); panel_text_w2 = std::max(panel_text_w2, ts.width); }
                                            int text_w2 = panel_text_w2 + pad2*2;
                                            int text_h2 = static_cast<int>(lines2.size())*lh2 + pad2*2;
                                            auto params = [this](const char* key, int defv){return this->has_parameter(key)? this->get_parameter(key).as_int() : defv;};
                                            std::string anchor2 = this->has_parameter("info_anchor") ? this->get_parameter("info_anchor").as_string() : std::string("top_right");
                                            int offx2 = params("info_offset_x", 8);
                                            int offy2 = params("info_offset_y", 8);
                                            int anchor_x2 = (anchor2 == "top_left") ? roi.x : (roi.x + roi.width);
                                            int anchor_y2 = roi.y;
                                            int px_i2 = anchor_x2 + offx2;
                                            int py_i2 = anchor_y2 + offy2;
                                            px_i2 = std::clamp(px_i2, 0, std::max(0, output_image.cols - text_w2 - 1));
                                            py_i2 = std::clamp(py_i2, 0, std::max(0, output_image.rows - text_h2 - 1));
                                            cv::Rect info2(px_i2, py_i2, text_w2, text_h2);
                                            {
                                                cv::Mat roi_r2 = output_image(info2); cv::Mat ov_r2; roi_r2.copyTo(ov_r2);
                                                cv::rectangle(ov_r2, cv::Rect(0,0,info2.width,info2.height), cv::Scalar(0,0,0), -1);
                                                cv::addWeighted(ov_r2, 0.45, roi_r2, 0.55, 0.0, roi_r2);
                                            }
                                            cv::rectangle(output_image, info2, cv::Scalar(255,255,255), 1);
                                            int tx2 = info2.x + pad2; int ty2 = info2.y + pad2 + 12;
                                            for (const auto &ln : lines2) { fluent::text::draw(output_image, ln, cv::Point(tx2, ty2), cv::Scalar(255,255,255), 0.5, 1); ty2 += lh2; }
                                        }
                                    } catch (...) {}
                                }
                            }
