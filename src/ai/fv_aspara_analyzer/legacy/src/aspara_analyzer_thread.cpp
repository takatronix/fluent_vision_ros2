    ...
    while (!shutdown_flag_) {
        ...
        if (has_new_data_.load()) {
            ...
            try {
                ...
                // 解析ターゲットをオーバーレイエンジンへ即時供給（メイン描画はdt補間で追従）
                try { node_->overlay_engine_.setTarget(aspara_copy); } catch (...) {}
...
void AnalyzerThread::processAsparagus(AsparaInfo& aspara_info)
{
    ...
    try {
        ...
        if (!sliced_from_registered) {
            ...
            // 解析に使用したカラー画像をノード側に保存（描画ズレ抑制）
            try {
                node_->storeLastAnalyzedImage(color_mat, color_image->header);
            } catch (...) {}
        ...
        // ===== Approach point (2D + 3D) =====
        try {
            // 2D pixel
            ApproachParams ap{};
            ap.y_mode = ApproachYMode::CENTER_TO_BOTTOM_RATIO;
            ap.y_ratio = 0.75f;
            ap.x_offset_px = 0;
            ap.y_offset_px = 0;
            try {
                // declare_parameter を使うことで、YAMLにある値を未宣言でも取得できる
                std::string mode = node_->declare_parameter<std::string>("approach.y_mode", std::string("center_to_bottom_ratio"));
                if (mode == "center") ap.y_mode = ApproachYMode::CENTER;
                else if (mode == "root_scanline") ap.y_mode = ApproachYMode::ROOT_SCANLINE;
                else ap.y_mode = ApproachYMode::CENTER_TO_BOTTOM_RATIO;
            } catch (...) {}
            try { ap.y_ratio = static_cast<float>(node_->declare_parameter<double>("approach.y_ratio", 0.75)); } catch (...) {}
            try { ap.x_offset_px = node_->declare_parameter<int>("approach.x_offset_px", 0); } catch (...) {}
            try { ap.y_offset_px = node_->declare_parameter<int>("approach.y_offset_px", 0); } catch (...) {}

            // 計算保持用も検出矩形中心（bounding_box_2d の厳密中心）
            ...
            aspara_info.approach.pixel = px;
            aspara_info.approach.pixel_valid = (px.x >= 0.0f && px.y >= 0.0f);
            RCLCPP_WARN(rclcpp::get_logger("analyzer_thread"),
                "[APPROACH_COMPUTE] bbox=(%d,%d,%d,%d) center=(%.1f,%.1f)",
                b.x, b.y, b.width, b.height, px.x, px.y);
            aspara_info.approach.y_mode = ap.y_mode;
            aspara_info.approach.score = aspara_info.approach.pixel_valid ? 1.0f : 0.0f;

            // 3D world
            aspara_info.approach.world_valid = false;
            if (aspara_info.approach.pixel_valid) {
                cv::Mat depth_for_appr;
                try {
                    if (depth_image->encoding == sensor_msgs::image_encodings::TYPE_16UC1) {
                        depth_for_appr = cv_bridge::toCvCopy(depth_image, sensor_msgs::image_encodings::TYPE_16UC1)->image;
                    } else if (depth_image->encoding == sensor_msgs::image_encodings::TYPE_32FC1) {
                        depth_for_appr = cv_bridge::toCvCopy(depth_image, sensor_msgs::image_encodings::TYPE_32FC1)->image;
                    }
                } catch (...) {}
                if (!depth_for_appr.empty()) {
                    DepthParams dp{};
                    dp.window_px = node_->declare_parameter<int>("approach.depth.window_px", 5);
                    dp.min_valid = node_->declare_parameter<int>("approach.depth.min_valid", 5);
                    dp.strategy = node_->declare_parameter<std::string>("approach.depth.strategy", std::string("median"));
                    dp.search_expand_px = node_->declare_parameter<int>("approach.depth.search_expand_px", 8);
                    dp.max_range_m = static_cast<float>(node_->declare_parameter<double>("approach.depth.max_range_m", 2.0));

                    // カラーと深度の整列が無い場合、深度画像の解像度へスケーリングしてからサンプリング
                    cv::Point2f px_depth = px;
                    if (!node_->use_color_camera_info_flag_) {
                        // depth_image と color_image の解像度差に基づく単純スケール
                        float sx = 1.0f, sy = 1.0f;
                        if (color_image && color_image->width > 0 && color_image->height > 0) {
                            sx = static_cast<float>(depth_image->width) / static_cast<float>(color_image->width);
                            sy = static_cast<float>(depth_image->height) / static_cast<float>(color_image->height);
                        } else {
                            sx = static_cast<float>(depth_for_appr.cols) / std::max(1, depth_for_appr.cols);
                            sy = static_cast<float>(depth_for_appr.rows) / std::max(1, depth_for_appr.rows);
                        }
                        px_depth.x = std::round(px.x * sx);
                        px_depth.y = std::round(px.y * sy);
                        RCLCPP_WARN(rclcpp::get_logger("analyzer_thread"),
                            "[APPROACH_COMPUTE] depth_scale=(%.3f,%.3f) color_px=(%.1f,%.1f) depth_px=(%.1f,%.1f)",
                            sx, sy, px.x, px.y, px_depth.x, px_depth.y);
                    }

                    geometry_msgs::msg::Point w;
                    bool ok = computeApproachPoint3D(px_depth, depth_for_appr, node_->depth_unit_m_16u_, *camera_info, dp, w);
                    if (ok) { aspara_info.approach.world = w; aspara_info.approach.world_valid = true; }
                }
            }
        } catch (...) {
            RCLCPP_WARN(rclcpp::get_logger("analyzer_thread"), "[APPROACH] computation failed (exception)");
        }
        ...
        if (!base_cloud->points.empty()) {
            ...
            // 参照深度を z0 からアプローチポイント深度へ切替可能に（安全ガード付き）
            float z_ref = z0;
            try {
                bool use_appr = node_ && node_->has_parameter("back_cut.use_approach_depth") ?
                    node_->get_parameter("back_cut.use_approach_depth").get_value<bool>() : false;
                if (use_appr && aspara_info.approach.world_valid && std::isfinite(aspara_info.approach.world.z)) {
                    double max_delta = node_->has_parameter("back_cut.approach_max_delta_m") ?
                        node_->get_parameter("back_cut.approach_max_delta_m").get_value<double>() : 0.06;
                    bool use_min = node_->has_parameter("back_cut.use_min_with_z0") ?
                        node_->get_parameter("back_cut.use_min_with_z0").get_value<bool>() : true;
                    float z_appr = static_cast<float>(aspara_info.approach.world.z);
                    // アプローチ深度が z0 から極端に離れていないときのみ採用
                    if (std::isfinite(z0) && std::fabs(z_appr - z0) <= static_cast<float>(max_delta)) {
                        z_ref = use_min ? std::min(z0, z_appr) : z_appr;
                    } else {
                        // 逸脱が大きい場合は安全に z0 を継続
                        z_ref = z0;
                    }
                }
                // バイアスで少し手前に寄せる（正でカメラ側＝小さいZへ）
                double bias = node_->has_parameter("back_cut.z_ref_bias_m") ?
                    node_->get_parameter("back_cut.z_ref_bias_m").get_value<double>() : 0.0;
                if (std::isfinite(z_ref) && std::isfinite(bias)) {
                    z_ref = static_cast<float>(std::max(0.0, static_cast<double>(z_ref) - bias));
                }
            } catch (...) {}
            for (const auto& p : base_cloud->points) {
                if (!std::isfinite(p.z) || p.z <= 0.0f) continue;
                if (simple_cut) {
                    if ((p.z - z_ref) > z_back) continue; // 背面カットのみ
                } else {
                    if (std::fabs(p.z - z_ref) > dist_window) continue;
                }
...
void AnalyzerThread::updateAnalysisResult(const AsparaInfo& aspara_info, long processing_time_ms)
{
    ...
    if (it != node_->aspara_list_.end()) {
        ...
        it->skeleton_points = aspara_info.skeleton_points;  // 骨格ポイントも更新
            // アプローチポイントを転送（UI描画用）
            it->approach = aspara_info.approach;
        // ヒストグラム帯も転送
