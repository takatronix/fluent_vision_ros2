...
# 穏やかな停止
info "📦 Sending TERM to D415 processes..."
...
if echo "$ROS_NODES" | grep -qE '(fv_realsense_d415|fv_object_detector_d415|fv_object_mask_generator_d415|fv_aspara_analyzer_d415|fv_stem_detector_d415)'; then
  info "注意: ROSグラフにノードがまだ見えます（別ユーザ/別マシンの可能性）。"
else
  info "✅ D415-related nodes are no longer visible."
fi
