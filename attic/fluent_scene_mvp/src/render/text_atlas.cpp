...
bool TextAtlas::init(const Options& options, DiagnosticList& diagnostics) {
    const auto fail = [&](const std::string& message) {
        diagnostics.add("compile.font_unavailable", Severity::kError, Phase::kCompile, Span{}, message,
                        {{"font", options.font_file}});
        return false;
    };
    ...
    for (FT_Long index = 1; index < num_faces; ++index) {
        if (face->family_name != nullptr && std::string(face->family_name).find("JP") != std::string::npos) {
            break;
        }
    ...
    ft_face_ = face;
    if (FT_Set_Pixel_Sizes(face, 0, options.pixel_size) != 0) {
    ...
    replacement_advance_ = static_cast<float>(options.pixel_size) * 0.75f;
    return true;
}

void TextAtlas::makeReplacementBitmap(std::vector<uint8_t>& bitmap, uint32_t& w, uint32_t& h) const {
    // Deterministic hollow box (spec section 9: missing glyphs use a
    // deterministic replacement glyph and are reported through a counter).
...
void TextAtlas::blitToCell(uint32_t cell_index, const uint8_t* src, uint32_t src_w, uint32_t src_h,
                           int src_pitch) {
