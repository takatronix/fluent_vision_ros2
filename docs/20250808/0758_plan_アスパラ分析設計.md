...
### 1. 統合アーキテクチャ
- **1つのソースコード**: `fv_aspara_analyzer_node`
- **複数ノードインスタンス**: 起動時引数で設定ファイル指定（d415.yaml / d405.yaml）
- **マルチカメラ対応**: 同じソースから複数カメラ用ノードを同時起動
...
## 重要な実装方針変更（2025-08-08 11:40更新）

### フレーム毎分析処理アーキテクチャ
**変更理由**: 同じ指摘を繰り返し受けることを防ぐため
...
#### 実装方針
```cpp
// detectionCallback: 検出情報更新のみ、点群処理はスキップ
void FvAsparaAnalyzerNode::detectionCallback(...) {
    // アスパラガス情報の更新のみ
    updateAsparagusList(...);
    
    // 検出情報更新時は点群処理はスキップ（imageCallbackで毎フレーム実行）
    // processAsparagus(selected_aspara_info_); ← この処理は削除
}

// imageCallback: 毎フレーム分析処理を実行
...
```yaml
# ポイントクラウド処理制御
enable_pointcloud_processing: true     # ポイントクラウド処理有効化フラグ
```
...
**重要**: この信頼度調整は「穂の検出信頼度」ではなく、「そのアスパラの頭部として判断する信頼度」を下げている。

### ID管理とトラッキング

#### AsparaSelection統合
- IoU 70%閾値による同一オブジェクト判定
- スムーズアニメーション（fluent_lib使用）
- 距離優先（矩形サイズ）、信頼度次点での最適候補選定
...
#### 3. **情報の優先順位**
1. **基本情報**（常時表示）: ID、信頼度、距離
2. **処理情報**（選択中のみ）: 点群数、分析時間
3. **分析結果**（分析完了後）: 長さ、太さ、真直度、グレード
4. **穂情報**（検出時のみ）: 穂数
...
#### 6. **テキスト描画仕様**
- **選択中**: 白色テキスト、影付き（視認性向上）
- **非選択**: グレーテキスト、小さめフォント
- **フォントサイズ**: 選択中 0.6、非選択 0.4
- **行間隔**: 20px
- **矩形との余白**: 5px
...
## 実装履歴
- 2025-08-08 11:33: 穂密集度対応とID管理統合実装完了
- 2025-08-08 11:40: フレーム毎分析処理方針をドキュメント化